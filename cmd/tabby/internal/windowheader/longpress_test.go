...
// The armed timer must resolve to a right-click at the press position.
func TestLongPressTimerFiresAtPressPosition(t *testing.T) {
	...
	result, cmd := m.handleMouse(tea.MouseMsg{
		Action: tea.MouseActionPress, Button: tea.MouseButtonLeft, X: 7, Y: 2,
	})
	if cmd == nil {
	...
	if lp.X != 7 || lp.Y != 2 {
		t.Fatalf("long-press fired at (%d,%d), want (7,2)", lp.X, lp.Y)
	}

	// Movement past the threshold cancels it, so a drag is not a menu.
	moved, _ := result.(rendererModel).handleMouse(tea.MouseMsg{
		Action: tea.MouseActionMotion, X: 7 + movementThreshold + 1, Y: 2,
	})
