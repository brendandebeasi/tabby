// Package panepicker is an interactive pane picker TUI that selects a
// tmux pane by command/path filter. Exported as the `tabby pane-picker`
// subcommand.
package panepicker
...
func initialModel(windowID string) model {
...
func Run(args []string) int {
	fs := flag.NewFlagSet("pane-picker", flag.ContinueOnError)
	fs.SetOutput(os.Stderr)
	windowID := fs.String("window", "", "tmux window ID to list panes for")
	...
	p := tea.NewProgram(initialModel(*windowID), tea.WithAltScreen())
