...
- Hooks stop printing `... returned 1` into every attached client. `refresh-client -S` needs a current client, and a backgrounded `run-shell` fired by a hook whose originating client has already gone away has none. The refresh is a best-effort status repaint, so the failure is now swallowed.
