#!/usr/bin/env bash
# vm_shot_box.sh <graphics-mode> <out.png>   (mode: off|sixel|kitty|auto)
set -u
WT=/Users/b/git/tabby/.claude/worktrees/custom-pane-borders
MODE="${1:-off}"; OUT="${2:-$WT/scripts/_box.png}"
L=tbshot; DISP=":99"
...
# pty keepalive spawns the box reliably.
python3 -c "
...
# xterm as the sixel-capable render client; kill pty so xterm drives termfeatures.
...
sleep 3; kill $PTY >/dev/null 2>&1
[ -n "$DP" ] && kill -USR1 $DP; sleep 4
echo "mode=$MODE termfeatures=[$(tmux -L $L display -p '#{client_termfeatures}')] edges=$(tmux -L $L list-panes -t t -F '#{pane_start_command}' | grep -c 'render pane-border')"
DISPLAY=$DISP import -window root "$OUT" 2>/dev/null
echo "shot: $OUT ($(wc -c < "$OUT" 2>/dev/null) bytes)"
kill $XTERM $XVFB >/dev/null 2>&1; tmux -L $L kill-server >/dev/null 2>&1
